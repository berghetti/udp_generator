...
case $WK in
  ...
  "extreme") AVG_SERVICE_TIME=$(awk 'BEGIN {print 0.9*0.995 + 300*0.005 }') ;; #extreme 99.5%/0.5%
  "high") AVG_SERVICE_TIME=$(awk 'BEGIN {print 0.9*0.5 + 300*0.5 }') ;;   #high 50%/50%
esac
...
#for rate in ${RPS[@]: -2}; do
  ...
  for i in $(seq 0 $((N_TESTS-1))); do
    ...
    until $(dirname $0)/shremote.py $(dirname $0)/clients.yml $1 -- --basedir $BASE_DIR --rate $rate --wk $WK --rand $rand --test $i --policy $1
    do
