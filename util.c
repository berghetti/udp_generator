...
void wait_timeout() {
  ...
  while ((rte_rdtsc() - t0) < (2 * 1000000 * TICKS_PER_US))
    ;
  ...
  for (uint32_t i = 0; i < nr_queues; i++) {
    ...
    for (; j < incoming_idx; j++) {
      ...
      fprintf(fp, "%u\t%lu\n",
              ...
              // delay app -> tx cur->worker_rx, // worker id rx cur->worker_tx,
...
inline void fill_payload_pkt(struct rte_mbuf *pkt, enum payload_item item,
                             uint64_t value) {
